...
export const ChatMessage = component$<ChatMessageProps>(({ message, userInitials }) => {
  ...
  return (
    <div class={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      {/* Bubble */}
      <div class={`max-w-[75%] space-y-1.5 flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
        <div class={[
        ...
        <div class={`flex items-center gap-2 px-1 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
          ...
          {!isUser && message.model && (
            <span class="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-400 dark:bg-slate-800">
              {message.model}
            </span>
          )}
          {!isUser && message.cacheStatus === 'hit' && (
            <span class="rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:bg-emerald-950/60 dark:text-emerald-400">
              cached
            </span>
          )}
        </div>
